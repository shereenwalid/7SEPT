...
# ── Cost rates ────────────────────────────────────────────────────────────
# Defaults are the published list prices (USD) for Gemini 2.5 Pro and
# europe-west1 Cloud Run / BigQuery. Override any of them via env vars if you
# have negotiated rates.
#   Gemini 2.5 Pro : $1.25 /1M input, $10.00 /1M output  (prompts <= 200k)
#                    $2.50 /1M input, $15.00 /1M output  (prompts >  200k)
#   Cloud Run      : $0.000018 /vCPU-second, $0.000002 /GiB-second
#                    (europe-west1, instance-based billing / CPU always allocated)
#   BigQuery       : $7.50 /TiB scanned (europe-west1), min 10 MB per query,
#                    first 1 TiB/month free (not applied per-run)
RATE_IN_PER_1M      = float(os.getenv("RATE_IN_PER_1M", "1.25"))       # <=200k prompt
RATE_OUT_PER_1M     = float(os.getenv("RATE_OUT_PER_1M", "10.00"))     # <=200k prompt
RATE_IN_PER_1M_BIG  = float(os.getenv("RATE_IN_PER_1M_BIG", "2.50"))   # >200k prompt
RATE_OUT_PER_1M_BIG = float(os.getenv("RATE_OUT_PER_1M_BIG", "15.00")) # >200k prompt
BIG_PROMPT_TOKENS   = int(os.getenv("BIG_PROMPT_TOKENS", "200000"))
RATE_VCPU_SEC       = float(os.getenv("RATE_VCPU_SEC", "0.000018"))    # Cloud Run $/vCPU-s (europe-west1, instance-based)
RATE_GIB_SEC        = float(os.getenv("RATE_GIB_SEC", "0.000002"))     # Cloud Run $/GiB-s (europe-west1, instance-based)
RATE_BQ_PER_TIB     = float(os.getenv("RATE_BQ_PER_TIB", "7.50"))      # BigQuery $/TiB (europe-west1)
BQ_MIN_MB_PER_QUERY = float(os.getenv("BQ_MIN_MB_PER_QUERY", "10"))    # BigQuery 10MB floor
RUN_VCPU            = float(os.getenv("RUN_VCPU", "2"))                # Cloud Run vCPUs
RUN_GIB             = float(os.getenv("RUN_GIB", "4"))                 # Cloud Run memory GiB
COST_DIR            = os.getenv("COST_DIR", "cost_reports")
...
def _metrics_finish_and_print(opp_id: str):
    ...
    # Gemini: rate tier depends on prompt size (<=200k vs >200k tokens)
    ...
    tier_note = (">200k prompt tier" if big else "<=200k prompt tier")
    ...
    # BigQuery bills a minimum of 10 MB per query - apply the floor
    billed_mb = max(mb_read, m["bq_queries"] * BQ_MIN_MB_PER_QUERY)
    billed_tib = billed_mb * 1048576 / (1024 ** 4)
    cost_bq   = billed_tib * RATE_BQ_PER_TIB
    ...
    lines = [
        ...
        "",
        "RATES USED",
        f"  Gemini input   : {rate_in} per 1M tokens  ({tier_note})",
        f"  Gemini output  : {rate_out} per 1M tokens  ({tier_note})",
        ...
        f"  BigQuery       : {RATE_BQ_PER_TIB} per TiB scanned (min {BQ_MIN_MB_PER_QUERY:.0f} MB/query)",
        "",
        "COST PER SERVICE",
        "-" * 72,
        f"Gemini (input)      = {m['input_tokens']:,} / 1,000,000 x {rate_in}"
        f"  = {cost_in:.6f}",
        f"Gemini (output)     = {m['output_tokens']:,} / 1,000,000 x {rate_out}"
        f"  = {cost_out:.6f}",
        ...
        f"  = {cost_mem:.6f}",
        f"BigQuery (queries)  = {billed_mb:.2f} MB billed ({m['bq_queries']} queries, "
        f"{mb_read:.2f} MB scanned) x {RATE_BQ_PER_TIB}/TiB  = {cost_bq:.6f}",
    ...
    lines.append("NOTE: list prices (USD). Override via env vars if you have "
                 "negotiated rates. Free tiers not applied.")
    lines.append("NOTE: Cloud Run figure is an upper bound - it charges the full "
                 "instance to this one run. With container concurrency 80, "
                 "concurrent runs share the instance, so the real per-run share "
                 "is lower.")
    report = "\n".join(lines)
